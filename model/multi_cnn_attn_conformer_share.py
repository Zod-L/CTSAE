...
class decoder(nn.Module):
    ...
    def __init__(self, patch_size=16, embed_dim=768, base_channel=64, channel_ratio=4, num_med_block=0,
                 ...
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., im_size=224, first_up=2, num_branch=4):
        ...
        for i in range(num_branch):
            setattr(self, f"first_cnn_{i}", nn.Conv2d(base_channel, int(base_channel * channel_ratio * 4), kernel_size=1))
        ...
        for i in range(num_branch):
            setattr(self, f"trans_patch_conv_{i}", nn.Conv2d(int(base_channel * channel_ratio * 2 * 2), embed_dim, kernel_size=trans_dw_stride, stride=trans_dw_stride, padding=0))
        ...
        for i in range(init_stage, fin_stage):
            ...
            self.add_module('conv_trans_' + str(i),
                    ConvTransBlock(
                        in_channel, stage_1_channel, res_conv, s, dw_stride=trans_dw_stride * 2, embed_dim=embed_dim,
                        num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=self.trans_dpr[i-1],
        ...
        for i in range(init_stage, fin_stage):
            ...
            self.add_module('conv_trans_' + str(i),
                    ConvTransBlock(
                        in_channel, stage_2_channel, res_conv, s, dw_stride=trans_dw_stride * 2 * 2, embed_dim=embed_dim,
                        num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=self.trans_dpr[i-1],
                        num_med_block=num_med_block, decode=True
                    )
        ...
        for i in range(init_stage, fin_stage):
            ...
            self.add_module('conv_trans_' + str(i),
                    ConvTransBlock(
                        in_channel, channel, res_conv, s, dw_stride=trans_dw_stride * 2 * 2 * 2, embed_dim=embed_dim,
                        num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                        drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, drop_path_rate=self.trans_dpr[i-1],
        ...
        self.conv_last = ConvBlock(inplanes=stage_3_channel // 2, outplanes=self.last_channel, res_conv=True, stride=1, act_layer=nn.Tanh)
        self.trans_last = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                             qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=self.trans_dpr[0], act_layer=nn.Tanh
                             )
        
        self.expand_last = FCUUp(inplanes=embed_dim, outplanes=self.last_channel, up_stride=trans_dw_stride * 2 * 2 * 2 * 2, act_layer=nn.Tanh)

        self.confuse_last = ConvBlock(inplanes=self.last_channel, outplanes=3, res_conv=True, stride=1, act_layer=nn.Tanh)
    ...
    def forward(self, x):
        ...
        xs = []
        for i in range(self.num_branch):
            xs.append(getattr(self, f"first_cnn_{i}")(x))
        x = torch.cat(xs, 1)
        x = self.frist_up(x)
        ...
        for i in range(self.fin_stage):
            x, xt = eval('self.conv_trans_' + str(i))(x, xt)

        _, _, H, W = x.shape
        x = self.conv_last(x, return_x_2=False)
        xt = self.trans_last(xt)
        xt = self.expand_last(xt, H // self.dw_stride, W // self.dw_stride)
        x = self.confuse_last(x + xt, return_x_2=False)
        return x
...
class auto_encoder_multi_cnn_attn_share(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, decode_embed=384, base_channel=64, channel_ratio=4, num_med_block=0,
        ...
        self.decoder = decoder(patch_size=patch_size, base_channel=base_channel, 
                               ...
                               num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale, drop_rate=drop_rate, 
                               attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path_rate, im_size=im_size, first_up=first_up, num_branch=num_branch)
